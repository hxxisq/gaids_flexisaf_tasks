# GAIDS Week [X] – Airline Passenger Data Visualization

# Week [X] Internship Tasks

This repository contains the completed exercise for Week [X], which includes a data visualization script analyzing airline passenger trends.

## 1. Airline Passengers Data Visualization
A data analysis project using `pandas`, `matplotlib`, and `seaborn` to visualize airline passenger data from the built-in Seaborn `flights` dataset.
* **Data Preparation:** Aggregates monthly passenger counts into yearly totals.
* **Visualizations:** A 2x2 grid of plots including:
  - Line plot of total passengers per year
  - Scatter plot of monthly passengers over time
  - Histogram (with KDE) of passenger volume distribution
  - Box plot of passenger distribution by month
* **Usage:** Open `GAIDS_Weekipynb.ipynb` in Jupyter or Colab and run all cells to generate the plots. Requires `matplotlib` and `seaborn`.

## Demo Video
Watch the walkthrough here: [Loom Recording](https://www.loom.com/share/6a6f35cd5866466988236e7427a85455)

*Developed for GAIDS Internship — Week [X] Task.*
